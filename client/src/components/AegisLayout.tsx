...
const navItems = [
  ...
  { path: "/discovery", label: "AI Discovery", icon: Search, section: "Monitoring", badge: "discovery" },
  { path: "/vendor", label: "Vendor Transparency", icon: Users, section: "Monitoring" },
  { path: "/aaef", label: "AAEF Appraisals", icon: ClipboardList, section: "Intelligence" },
...
export default function AegisLayout({ children, title }: AegisLayoutProps) {
  ...
  const { data: discoveryNewCount } = trpc.discovery.newCount.useQuery(undefined, { enabled: isAuthenticated });
  const pendingCount = stats?.pendingApprovals ?? 0;
  const unreadCount = stats?.unreadNotifications ?? 0;
  const discoveryCount = discoveryNewCount ?? 0;
              ...
              <div className="space-y-0.5">
                {visibleItems
                  ...
                  .map((item) => {
                    ...
                    const badgeCount = item.badge === "pending" ? pendingCount : item.badge === "unread" ? unreadCount : item.badge === "discovery" ? discoveryCount : 0;
