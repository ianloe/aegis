...
import { getPiholeSettings, pullPiholeFindings, savePiholeSettings } from "./pihole";
import {
  calculateWAS,
  checkOverrides,
  createAppraisal,
  createImprovementPlan,
  getAaefDashboardStats,
  getAppraisalById,
  getAppraisalsForAgent,
  getImprovementPlans,
  getProfiles,
...
import {
  ...
  upsertShadowAiTool,
  createDiscoveryScan,
  completeDiscoveryScan,
  getDiscoveryScans,
  createDiscoveryFinding,
  getDiscoveryFindings,
  updateDiscoveryFinding,
  countNewDiscoveryFindings,
} from "./db";
  ...
  agents: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const apiKey = `agt_${randomUUID().replace(/-/g, "")}`;
        const agent = await createAgent({ ...input, tenantId, status: "active", apiKey });
    ...
    update: protectedProcedure
      .input(z.object({
        ...
        version: z.string().optional(),
        owner: z.string().optional(),
        maxDataTier: z.enum(["benign", "internal", "sensitive"]),
      }))
      .mutation(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        const { id, ...data } = input;
        await updateAgent(id, tenantId, data);
        await createAuditLog({
          tenantId,
          agentId: id,
          agentName: input.name,
          ...
          summary: `Agent "${input.name}" updated`,
          dataTier: input.maxDataTier,
        });
        return { success: true };
      }),
    ...
    regenerateApiKey: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { randomUUID } = await import("crypto");
        const apiKey = `agt_${randomUUID().replace(/-/g, "")}`;
        await updateAgent(input.id, tenantId, { apiKey });
        return { apiKey };
      }),
...
Write a 3-4 paragraph executive summary covering: overall compliance posture, key gaps identified, risk areas, and recommended next steps. Use professional language suitable for a board-level audience.`;
  ...
  aaef: router({
    ...
    listProfiles: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      return getProfiles(tenantId);
    }),

    // ── Appraisals ────────────────────────────────────────────────────────────
    listAppraisals: protectedProcedure
      .input(z.object({ agentId: z.number() }))
      .query(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        return getAppraisalsForAgent(tenantId, input.agentId);
      }),

    getAppraisal: protectedProcedure
      .input(z.object({ appraisalId: z.number() }))
      .query(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        const appraisal = await getAppraisalById(tenantId, input.appraisalId);
        if (!appraisal) throw new TRPCError({ code: "NOT_FOUND" });
        return appraisal;
      }),

    createAppraisal: protectedProcedure
      .input(
        z.object({
          agentId: z.number(),
          profileId: z.number(),
          ...
          d1Score: z.number().int().min(1).max(5),
          d2Score: z.number().int().min(1).max(5),
          d3Score: z.number().int().min(1).max(5),
          d4Score: z.number().int().min(1).max(5),
          d5Score: z.number().int().min(1).max(5),
          d1Rationale: z.string().optional(),
          d2Rationale: z.string().optional(),
          d3Rationale: z.string().optional(),
          d4Rationale: z.string().optional(),
          d5Rationale: z.string().optional(),
          quantitativeDataSummary: z.string().optional(),
          nextAppraisalDate: z.string().optional(),
          consecutiveLowWas: z.number().int().min(0).default(0),
          improvementActions: z.array(
            z.object({
              dimension: z.enum(["D1", "D2", "D3", "D4", "D5", "overall"]),
              actionDescription: z.string(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const appraisal = await createAppraisal({
          ...
          conductedBy: ctx.user.name ?? ctx.user.username,
          d1Score: input.d1Score,
          d2Score: input.d2Score,
          d3Score: input.d3Score,
          d4Score: input.d4Score,
          d5Score: input.d5Score,
          d1Rationale: input.d1Rationale,
          d2Rationale: input.d2Rationale,
          d3Rationale: input.d3Rationale,
          d4Rationale: input.d4Rationale,
          d5Rationale: input.d5Rationale,
          quantitativeDataSummary: input.quantitativeDataSummary,
          nextAppraisalDate: input.nextAppraisalDate ? new Date(input.nextAppraisalDate) : undefined,
          consecutiveLowWas: input.consecutiveLowWas,
        });

        // Create improvement plan actions if provided
        for (const action of input.improvementActions) {
          await createImprovementPlan({
            ...
            checkInDate: action.checkInDate ? new Date(action.checkInDate) : undefined,
          });
        }

        return {
          appraisal,
          mandatoryResponse: ratingToMandatoryResponse(appraisal.overallRating),
        };
      }),

    // ── Improvement Plans ─────────────────────────────────────────────────────
    listImprovementPlans: protectedProcedure
      .input(z.object({ agentId: z.number() }))
      .query(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        return getImprovementPlans(tenantId, input.agentId);
      }),
    ...
    updatePlanStatus: protectedProcedure
      .input(
        z.object({
          planId: z.number(),
          status: z.enum(["open", "in_progress", "completed", "overdue", "escalated"]),
          completionNote: z.string().optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        await updateImprovementPlanStatus(
          tenantId,
          input.planId,
          input.status,
    ...
    dashboardStats: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      return getAaefDashboardStats(tenantId);
    }),

    // ── Utility: preview WAS without saving ──────────────────────────────────
    previewWAS: protectedProcedure
      .input(
        z.object({
          profileId: z.number(),
          d1Score: z.number().int().min(1).max(5),
      ...
      .query(async ({ ctx, input }) => {
        ...
        const scores = {
          d1: input.d1Score, d2: input.d2Score, d3: input.d3Score,
          d4: input.d4Score, d5: input.d5Score,
        };
        const was = calculateWAS(scores, weights);
        const rating = wasToRating(was);
        const overrides = checkOverrides(scores, was, input.consecutiveLowWas);
        return {
          was,
          rating,
          overrides,
          mandatoryResponse: ratingToMandatoryResponse(rating),
        };
      }),
  }),
  ...
  discovery: router({
    ...
    // Return the static AI service catalogue so the frontend can display it
    catalogue: protectedProcedure.query(() => AI_SERVICE_CATALOGUE),

    // List past scans for the tenant
    scans: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      return getDiscoveryScans(tenantId);
    }),

    // List findings, optionally filtered by scanId and/or status
    findings: protectedProcedure
      .input(z.object({
        scanId: z.number().optional(),
        status: z.enum(["new", "reviewed", "promoted_agent", "added_shadow", "dismissed"]).optional(),
      }))
      .query(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        return getDiscoveryFindings(tenantId, input.scanId, input.status);
      }),

    // Count of new (unreviewed) findings — used for sidebar badge
    newCount: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      return countNewDiscoveryFindings(tenantId);
    }),

    // ── Scan: Endpoint Probe ────────────────────────────────────────────────
    runEndpointProbe: protectedProcedure.mutation(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      const scan = await createDiscoveryScan({
        tenantId,
        scanType: "endpoint_probe",
        triggeredBy: ctx.user.name ?? ctx.user.username,
        inputSummary: `Probing ${AI_SERVICE_CATALOGUE.length} known AI service endpoints`,
      });
      if (!scan) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create scan" });

      const start = Date.now();
      try {
        // Get registered agent endpoints to exclude from findings
        const registeredAgents = await getAgents(tenantId);
        const registeredEndpoints = registeredAgents
          .map((a) => a.vendor ?? "")
          .filter(Boolean);

        const results = await runEndpointProbe(registeredEndpoints);
        let findingsCount = 0;

        for (const r of results) {
          await createDiscoveryFinding({
            tenantId,
            scanId: scan.id,
            toolName: r.entry.toolName,
            vendor: r.entry.vendor,
            category: r.entry.category,
            endpoint: r.entry.endpoint,
            detectionMethod: r.httpStatus ? "http_probe" : "dns_probe",
            confidence: r.httpStatus && r.httpStatus < 500 ? "high" : "medium",
            evidence: r.httpStatus
              ? `HTTP ${r.httpStatus} from ${r.entry.endpoint} in ${r.responseTimeMs}ms`
              : `Endpoint ${r.entry.endpoint} reachable (no HTTP status)`,
            riskLevel: r.entry.riskLevel,
            riskRationale: r.entry.riskRationale,
          });
          findingsCount++;
        }

        await completeDiscoveryScan(scan.id, findingsCount, Date.now() - start);
        await createAuditLog({
          tenantId,
          userName: ctx.user.name ?? ctx.user.username,
          actionType: "anomaly_detected",
          summary: `Discovery scan (endpoint probe) completed: ${findingsCount} reachable AI endpoints found`,
          dataTier: "internal",
        });
        return { scanId: scan.id, findingsCount };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        await completeDiscoveryScan(scan.id, 0, Date.now() - start, msg);
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: msg });
      }
    }),

    // ── Scan: Log Analysis ──────────────────────────────────────────────────
    runLogAnalysis: protectedProcedure
      .input(z.object({ logText: z.string().min(10).max(50000) }))
      .mutation(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        const scan = await createDiscoveryScan({
          tenantId,
          scanType: "log_analysis",
          triggeredBy: ctx.user.name ?? ctx.user.username,
          inputSummary: `Analysing ${input.logText.length} characters of log text`,
        });
        if (!scan) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create scan" });

        const start = Date.now();
        try {
          const findings = await analyseLogText(input.logText);
          let findingsCount = 0;
          ...
          await createAuditLog({
            ...
            userName: ctx.user.name ?? ctx.user.username,
            actionType: "anomaly_detected",
            summary: `Discovery scan (log analysis) completed: ${findingsCount} AI tools identified in logs`,
            dataTier: "internal",
          });
          return { scanId: scan.id, findingsCount };
        } catch (err: unknown) {
    ...
    runAuditFingerprint: protectedProcedure.mutation(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      const scan = await createDiscoveryScan({
        tenantId,
        scanType: "audit_fingerprint",
        triggeredBy: ctx.user.name ?? ctx.user.username,
        inputSummary: "Cross-referencing audit logs for unregistered AI API patterns",
      });
      if (!scan) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create scan" });
      ...
      try {
        ...
        const registeredAgents = await getAgents(tenantId);
        const registeredNames = registeredAgents.map((a) => a.name);
        const summaries = logs.map((l: { summary: string | null; details: unknown }) => `${l.summary ?? ""} ${JSON.stringify(l.details ?? {})}`).filter(Boolean);

        const findings = fingerprintAuditLogs(summaries, registeredNames);
        let findingsCount = 0;
        ...
        await createAuditLog({
          ...
          userName: ctx.user.name ?? ctx.user.username,
          actionType: "anomaly_detected",
          summary: `Discovery scan (audit fingerprint) completed: ${findingsCount} unregistered AI patterns found`,
          dataTier: "internal",
        });
        return { scanId: scan.id, findingsCount };
      } catch (err: unknown) {
    ...
    // ── Promote finding → register as agent ────────────────────────────────
    promoteToAgent: protectedProcedure
      .input(z.object({
        findingId: z.number(),
        name: z.string().min(1),
        description: z.string().optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Get finding details for vendor/category
        const findings = await getDiscoveryFindings(tenantId, undefined, undefined);
        ...
        const agent = await createAgent({
          tenantId,
          name: input.name,
          description: input.description ?? finding.riskRationale ?? undefined,
          agentType: finding.category ?? undefined,
          vendor: finding.vendor ?? undefined,
          owner: input.owner,
        ...
        await updateDiscoveryFinding(input.findingId, tenantId, {
          status: "promoted_agent",
          reviewedBy: ctx.user.name ?? ctx.user.username,
          reviewNote: `Promoted to registered agent: ${input.name}`,
          promotedAgentId: agent?.id,
        });
        ...
        await createAuditLog({
          ...
          userName: ctx.user.name ?? ctx.user.username,
          actionType: "agent_registered",
          summary: `Agent "${input.name}" registered from discovery finding`,
          dataTier: input.maxDataTier,
        });
    ...
    // ── Promote finding → add to shadow AI registry ─────────────────────────
    promoteToShadow: protectedProcedure
      .input(z.object({
        findingId: z.number(),
        notes: z.string().optional(),
      }))
      .mutation(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        const findings = await getDiscoveryFindings(tenantId);
        const finding = findings.find((f) => f.id === input.findingId);
        if (!finding) throw new TRPCError({ code: "NOT_FOUND", message: "Finding not found" });

        const tool = await upsertShadowAiTool({
          tenantId,
          name: finding.toolName,
          vendor: finding.vendor ?? undefined,
          category: finding.category ?? undefined,
          detectedBy: "Discovery Engine",
          notes: input.notes ?? finding.riskRationale ?? undefined,
        });

        await updateDiscoveryFinding(input.findingId, tenantId, {
          status: "added_shadow",
          reviewedBy: ctx.user.name ?? ctx.user.username,
          reviewNote: "Added to shadow AI registry",
          shadowToolId: (tool as unknown as { id?: number })?.id,
        });
    ...
    dismiss: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await updateDiscoveryFinding(input.findingId, tenantId, {
          status: "dismissed",
          reviewedBy: ctx.user.name ?? ctx.user.username,
          reviewNote: input.reviewNote ?? "Dismissed as false positive",
        });
        return { success: true };
      }),
  }),
  ...
  pihole: router({
    getSettings: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      return getPiholeSettings(tenantId);
    }),

    saveSettings: protectedProcedure
      .input(
        z.object({
          url: z.string().url("Must be a valid URL, e.g. http://10.0.5.24"),
      ...
      .mutation(async ({ ctx, input }) => {
        const tenantId = await getTenantId(ctx.user.id);
        await savePiholeSettings(tenantId, input);
        return { success: true };
      }),

    pullNow: protectedProcedure.mutation(async ({ ctx }) => {
      const tenantId = await getTenantId(ctx.user.id);
      const result = await pullPiholeFindings(tenantId);
      return result;
    }),
  }),
});
